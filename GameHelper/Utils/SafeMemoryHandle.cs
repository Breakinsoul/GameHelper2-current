...
namespace GameHelper.Utils
{
    ...
    internal class SafeMemoryHandle : SafeHandleZeroOrMinusOneIsInvalid
    {
            ...
            where TValue : unmanaged
        {
            ...
            if (nativeContainer.Size <= 128)
            {
                while (first64Childrens.Count > 0)
                {
                    totalChildrenProcessed += processSubTree(first64Childrens, first64Childrens.Dequeue(), maxSizeAllowed);
                }
