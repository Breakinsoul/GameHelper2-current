...
namespace GameHelper.RemoteObjects
{
    ...
    public class LoadedFiles : RemoteObjectBase
    {
        ...
        internal override void ToImGui()
        {
            ...
            {
                this.searchTextSplit = this.searchText.Split(",", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
            }
            ...
            if (!string.IsNullOrEmpty(this.searchText))
            {
                if (ImGui.BeginChild("Result##loadedfiles", Vector2.Zero, ImGuiChildFlags.Borders))
                {
                    ...
                    foreach (var kv in this.PathNames)
                    {
                        ...
                        {
                            if (!kv.Key.Contains(this.searchTextSplit[i], StringComparison.OrdinalIgnoreCase))
                            {
                                containsAll = false;
                                break;
                            }
