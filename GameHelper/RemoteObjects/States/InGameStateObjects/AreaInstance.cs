...
namespace GameHelper.RemoteObjects.States.InGameStateObjects
{
    ...
    public class AreaInstance : RemoteObjectBase
    {
        ...
        protected override void UpdateData(bool hasAddressChanged)
        {
            ...
            PerformanceHitchDetector.RecordSection(
                "AreaInstance: Read offsets",
                System.Diagnostics.Stopwatch.GetTimestamp() - updateStart);

            if (hasAddressChanged)
            {
                var areaChangeStart = System.Diagnostics.Stopwatch.GetTimestamp();
                this.Cleanup(true);
                this.TerrainMetadata = data.TerrainMetadata;
                this.CurrentAreaLevel = data.CurrentAreaLevel;
                this.AreaHash = $"{data.CurrentAreaHash:X}";
                PerformanceHitchDetector.RecordSection(
                    "AreaInstance rebuild: Metadata",
                    System.Diagnostics.Stopwatch.GetTimestamp() - areaChangeStart);

                var walkableStart = System.Diagnostics.Stopwatch.GetTimestamp();
                this.GridWalkableData = reader.ReadStdVector<byte>(
                    this.TerrainMetadata.GridWalkableData);
                PerformanceHitchDetector.RecordSection(
                    "AreaInstance rebuild: Walkable",
                    System.Diagnostics.Stopwatch.GetTimestamp() - walkableStart);

                var heightStart = System.Diagnostics.Stopwatch.GetTimestamp();
                this.GridHeightData = this.GetTerrainHeight();
                PerformanceHitchDetector.RecordSection(
                    "AreaInstance rebuild: Terrain height",
                    System.Diagnostics.Stopwatch.GetTimestamp() - heightStart);

                if (Core.GHSettings.LoadTgtTileData)
                {
                    var tgtStart = System.Diagnostics.Stopwatch.GetTimestamp();
                    this.TgtTilesLocations = this.GetTgtFileData();
                    PerformanceHitchDetector.RecordSection(
                        "AreaInstance rebuild: TGT tiles",
                        System.Diagnostics.Stopwatch.GetTimestamp() - tgtStart);
                }
                else
                {
                    this.TgtTilesLocations = new();
                    PerformanceHitchDetector.RecordSectionMs("AreaInstance rebuild: TGT skipped", 0);
                }

                PerformanceHitchDetector.RecordSection(
                    "AreaInstance: Area changed rebuild",
                    System.Diagnostics.Stopwatch.GetTimestamp() - areaChangeStart);
            }

            var envStart = System.Diagnostics.Stopwatch.GetTimestamp();
            this.UpdateEnvironmentAndCaches(data.Environments);
            PerformanceHitchDetector.RecordSection(
                "AreaInstance: Environments",
                System.Diagnostics.Stopwatch.GetTimestamp() - envStart);

            var playerStart = System.Diagnostics.Stopwatch.GetTimestamp();
            this.ServerDataObject.Address = data.PlayerInfo.ServerDataPtr;
            this.Player.Address = data.PlayerInfo.LocalPlayerPtr;
            PerformanceHitchDetector.RecordSection(
                "AreaInstance: Player data",
                System.Diagnostics.Stopwatch.GetTimestamp() - playerStart);

            var entitiesStart = System.Diagnostics.Stopwatch.GetTimestamp();
            this.UpdateEntities(data.Entities.AwakeEntities, this.AwakeEntities, true);
            PerformanceHitchDetector.RecordSection(
                "AreaInstance: Entities",
                System.Diagnostics.Stopwatch.GetTimestamp() - entitiesStart);
        }
        ...
        private Dictionary<string, List<Vector2>> GetTgtFileData()
        {
            ...
            Parallel.For(
                ...
                (tileNumber, _, localstate) =>
                {
                    ...
                    if (localstate.TryGetValue(tgtName, out var locations))
                    {
                        locations.Add(loc);
                    }
                    else
        ...
        private float[][] GetTerrainHeight()
        {
            ...
            {
                var row = new float[gridSizeX];
                result[y] = row;
                var tileRowIndex = (y / TileStructure.TileToGridConversion) * (int)this.TerrainMetadata.TotalTiles.X;
                var gridYremaining = y % TileStructure.TileToGridConversion;
                for (var x = 0; x < gridSizeX; x++)
                {
                    var tileDataIndex = tileRowIndex + x / TileStructure.TileToGridConversion;
                    if (tileDataIndex < tileData.Length)
                    {
                        ...
                        if (subTileHeightCache.TryGetValue(mytiledata.SubTileDetailsPtr, out var subTileHeightsArray))
                        {
                            ...
                            var finalRotatedX = GetRotatorMatrixValue(rotatedX0 * 2 + rotatedX1, gridXremaining, gridYremaining);
                            var finalRotatedY = GetRotatorMatrixValue(rotatedY0 + rotatedY1, gridXremaining, gridYremaining);
                            var subTileHeight = this.GetSubTerrainHeight(subTileHeightsArray, finalRotatedY, finalRotatedX);
                            row[x] = (mytiledata.TileHeight * (float)this.TerrainMetadata.TileHeightMultiplier + subTileHeight) *
                                     TerrainStruct.TileHeightFinalMultiplier *
                                     -1;
                        }
                    }
                }
        ...
        private static int GetRotatorMatrixValue(int index, int gridXremaining, int gridYremaining)
        {
            return index switch
            {
                0 => TileStructure.TileToGridConversion - gridXremaining - 1,
                1 => gridXremaining,
                2 => TileStructure.TileToGridConversion - gridYremaining - 1,
                3 => gridYremaining,
                _ => 0,
            };
        }
