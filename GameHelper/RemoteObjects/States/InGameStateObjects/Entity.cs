...
namespace GameHelper.RemoteObjects.States.InGameStateObjects
{
    ...
    public class Entity : RemoteObjectBase
    {
        ...
        {
            return this.componentAddresses.ContainsKey(componentName);
        }
        ...
        {
            if (this.TryGetComponent<Chest>(out var _))
            {
            ...
            else if (this.HasComponent(nameof(Life)))
            {
                if (this.HasComponent(nameof(TriggerableBlockage)))
                {
                    return false;
                }

                var hasPositioned = this.TryGetComponent<Positioned>(out var pos);
                var isFriendly = hasPositioned && pos != null && pos.IsFriendly;
                var hasMonsterSignals =
                    this.Path.StartsWith("Metadata/Monsters/") ||
                    this.HasComponent("Monster") ||
                    this.HasComponent(nameof(Buffs)) ||
                    this.HasComponent(nameof(ObjectMagicProperties)) ||
                    this.HasComponent(nameof(Targetable)) ||
                    this.HasComponent(nameof(Stats)) ||
                    this.HasComponent("BaseEvents");

                if (!isFriendly && this.HasComponent(nameof(DiesAfterTime)))
                {
                    if (this.HasComponent(nameof(Targetable)))
                    {
                        this.EntityType = EntityTypes.Monster;
                    }
                    else
                ...
                else if (!isFriendly && hasMonsterSignals)
                {
                    this.EntityType = EntityTypes.Monster;
                }
                else
        ...
        private void CalculateEntityState()
        {
            ...
            {
                var isTargetable = this.TryGetComponent<Targetable>(out var targetableComp, false) &&
                    targetableComp.IsTargetable;

                var hasDead = this.TryGetStatValue(GameStats.is_dead, out var isDead);
                if (!isTargetable && hasDead && isDead > 0)
                {
                    this.EntityState = EntityStates.Useless;
                    return;
                }
        ...
        public bool TryGetStatValue(GameStats stat, out int value)
        {
            value = 0;
