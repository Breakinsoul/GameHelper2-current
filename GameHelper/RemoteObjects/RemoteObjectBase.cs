...
namespace GameHelper.RemoteObjects
{
    ...
    public abstract class RemoteObjectBase
    {
        ...
        private static readonly ConcurrentDictionary<(Type Type, BindingFlags Flags), PropertyInfo[]> ImGuiPropertyCache = new();
        private static readonly BindingFlags ToImGuiMethodFlags = BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public;
        ...
        {
            var properties = ImGuiPropertyCache.GetOrAdd(
                (classType, propertyFlags),
                key => key.Type.GetProperties(key.Flags));
            for (var i = 0; i < properties.Length; i++)
            {
                ...
                yield return new RemoteObjectPropertyDetail
                {
                    Name = property.Name,
                    Value = propertyValue,
                    ToImGui = propertyType.GetMethod("ToImGui", ToImGuiMethodFlags)
                };
            }
        }
