...
namespace GameHelper
{
    ...
    public class GameProcess
    {
        ...
        /// <summary>
        ///     Gets the main module file name for the attached game process, if available.
        /// </summary>
        public string MainModuleFileName
        {
            get
            {
                try
                {
                    return this.Information?.MainModule?.FileName ?? string.Empty;
                }
                catch (Exception)
                {
                    return string.Empty;
                ...
                foreach (var process in Process.GetProcesses())
                {
                    try
                    {
                        if (GameProcessDetails.ProcessName.TryGetValue(process.ProcessName, out var windowTitle) &&
                            string.Equals(process.MainWindowTitle, windowTitle, StringComparison.OrdinalIgnoreCase))
                        {
        ...
        private void DisposeCachedProcesses()
        {
            foreach (var process in this.processesInfo)
            {
                if (!ReferenceEquals(process, this.Information))
                {
                    process.Dispose();
                }
            }
