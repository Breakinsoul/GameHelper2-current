...
public static class PerformanceProfiler
{
    ...
    private static List<ProfileRow> cachedSortedRows = [];
    private static bool showCurrentFrameOnly = true;
    private static int lastSortColumn = 3;
    private static bool lastSortAscending = false;
    private static bool rowsDirty = true;
            ...
            if (ImGui.Begin("Performance Profiler", ref Core.GHSettings.ShowPerfProfiler, ImGuiWindowFlags.MenuBar))
            {
                if (ImGui.BeginMenuBar())
                {
                    if (ImGui.MenuItem("Reset"))
                    {
                        ProfileData.Clear();
                        cachedRows.Clear();
                        cachedSortedRows.Clear();
                        rowsDirty = true;
                    }
                    if (ImGui.Checkbox("Current Frame Only", ref showCurrentFrameOnly))
                    {
                        cachedRows.Clear();
                        cachedSortedRows.Clear();
                        rowsDirty = true;
                        ...
                        ImGui.GetContentRegionAvail()))
                {
                    ...
                    if (rowsDirty || sortColumn != lastSortColumn || sortAscending != lastSortAscending)
                    {
                        cachedSortedRows = sortColumn switch
                        {
                            0 => sortAscending ? cachedRows.OrderBy(r => r.Count).ToList() : cachedRows.OrderByDescending(r => r.Count).ToList(),
                            1 => sortAscending ? cachedRows.OrderBy(r => r.Name).ToList() : cachedRows.OrderByDescending(r => r.Name).ToList(),
                            2 => sortAscending ? cachedRows.OrderBy(r => r.AvgPerCallNs).ToList() : cachedRows.OrderByDescending(r => r.AvgPerCallNs).ToList(),
                            3 => sortAscending ? cachedRows.OrderBy(r => r.AvgPerFrameNs).ToList() : cachedRows.OrderByDescending(r => r.AvgPerFrameNs).ToList(),
                    ...
                    foreach (var row in cachedSortedRows)
                    {
                        ImGui.TableNextRow();
