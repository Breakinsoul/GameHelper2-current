...
namespace GameHelper
{
    ...
    public static class Core
    {
        ...
        internal static void Dispose()
        {
            try
            {
                JsonHelper.SafeToFile(GHSettings, State.CoreSettingFile);
                PManager.ShutdownPlugins();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Core.Dispose] Failed to save settings during shutdown: {ex}");
            }
