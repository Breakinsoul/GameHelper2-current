...
Compress-Archive -Path (Join-Path $out "*") -DestinationPath "artifacts\GameHelper-v2.3.7.zip" -Force
```
...
```text
GameHelper-v2.3.7/GameHelper.exe
```
...
```powershell
gh release create v2.3.7 artifacts\GameHelper-v2.3.7.zip `
  -R Breakinsoul/GameHelper2-current `
  --title "v2.3.7 radar interpolation" `
...
2. Publish a newer release, for example `v2.3.7`.
