...
namespace Radar
{
    ...
    public sealed class Radar : PCore<RadarSettings>
    {
        ...
        private DateTime lastRadarMonitorWriteTime = DateTime.MinValue;
        private Vector2 lastLargeMapCenter = Vector2.Zero;
        private Vector2 lastMiniMapCenter = Vector2.Zero;
        ...
        private static string RadarMonitorPath => "radar_monitor.jsonl";
        ...
        public override void DrawUI()
        {
            ...
            {
                if (!this.TryCreateRadarFrame(out var frame))
                {
                    return;
                }
                ...
                ImGui.PopStyleVar();
                this.WriteRadarMonitorFileIfNeeded("large", largeMapRealCenter, largeMap.Shift, largeMap.DefaultShift, largeMap.Zoom, frame);
                this.DrawLargeMap(largeMapRealCenter, frame);
                this.DrawTgtFiles(largeMapRealCenter, frame);
                this.DrawTgtIcons(largeMapRealCenter, frame, largeMapModifiedZoom * 5f);
                this.DrawMapIcons(largeMapRealCenter, frame, largeMapModifiedZoom * 5f);
                ImGui.End();
            }
            ...
            if (miniMap.IsVisible)
            {
                ...
                ImGui.PopStyleVar();
                this.WriteRadarMonitorFileIfNeeded("mini", miniMapCenter, miniMap.Shift, miniMap.DefaultShift, miniMap.Zoom, frame);
                this.DrawTgtIcons(miniMapCenter, frame, miniMap.Zoom);
                this.DrawMapIcons(miniMapCenter, frame, miniMap.Zoom);
                ImGui.End();
            }
        }
        ...
        private bool TryCreateRadarFrame(out RadarFrame frame)
        {
            frame = default;
            ...
            var entities = new List<RadarEntityFrame>(currentAreaInstance.AwakeEntities.Count);
            foreach (var entity in currentAreaInstance.AwakeEntities)
            {
                ...
                entities.Add(new RadarEntityFrame(
                    entityValue,
                    entityValue.IsValid,
                    entityValue.EntityType,
                    entityValue.EntitySubtype,
                    entityValue.EntityState,
                    entityValue.EntityCustomGroup,
                    entityValue.Path,
                    new Vector2(entityRender.GridPosition.X, entityRender.GridPosition.Y),
                    entityRender.TerrainHeight));
            }

            frame = new RadarFrame(
                new Vector2(playerRender.GridPosition.X, playerRender.GridPosition.Y),
                playerRender.TerrainHeight,
                entities.ToArray());
            return true;
        }

        private void DrawLargeMap(Vector2 mapCenter, RadarFrame frame)
        {
            if (!this.Settings.DrawWalkableMap)
            {
                return;
            ...
            var p1 = Helper.DeltaInWorldToMapDelta(
                new Vector2(rectf.Left, rectf.Top), -frame.PlayerTerrainHeight);
            var p2 = Helper.DeltaInWorldToMapDelta(
                new Vector2(rectf.Right, rectf.Top), -frame.PlayerTerrainHeight);
            var p3 = Helper.DeltaInWorldToMapDelta(
                new Vector2(rectf.Right, rectf.Bottom), -frame.PlayerTerrainHeight);
            var p4 = Helper.DeltaInWorldToMapDelta(
                new Vector2(rectf.Left, rectf.Bottom), -frame.PlayerTerrainHeight);
            p1 += mapCenter;
            p2 += mapCenter;
            p3 += mapCenter;
            p4 += mapCenter;
        ...
        private void DrawTgtFiles(Vector2 mapCenter, RadarFrame frame)
        {
            ...
            void drawString(string text, Vector2 location, Vector2 stringImGuiSize, bool drawBackground)
            {
                ...
                var fpos = Helper.DeltaInWorldToMapDelta(
                    location - pPos, -frame.PlayerTerrainHeight + height);
        ...
        private void DrawTgtIcons(Vector2 mapCenter, RadarFrame frame, float iconSizeMultiplier)
        {
            ...
            foreach (var tgtKV in currentAreaInstance.TgtTilesLocations)
            {
                if (tgtKV.Key.StartsWith(TempleTgtPrefix) && tgtKV.Key.EndsWith(":1-y:1"))
                {
                    ...
                    this.DrawIconAtTgtLocations(fgDraw, mapCenter, pPos, frame.PlayerTerrainHeight, tgtKV.Value, templeIcon, iconSizeMultiplier, shiftUp: true);
                }
                else if (this.Settings.BossArenaTgts.ContainsKey(tgtKV.Key))
                {
                    ...
                    this.DrawIconAtTgtLocations(fgDraw, mapCenter, pPos, frame.PlayerTerrainHeight, tgtKV.Value, bossIcon, iconSizeMultiplier);
                }
                else if (this.Settings.StairsTgts.ContainsKey(tgtKV.Key))
                {
                    ...
                    this.DrawIconAtTgtLocations(fgDraw, mapCenter, pPos, frame.PlayerTerrainHeight, tgtKV.Value, stairsIcon, iconSizeMultiplier);
                }
            }
        }
            ...
            bool shiftUp = false)
        {
            ...
            for (var i = 0; i < locations.Count; i++)
            {
                ...
                var fpos = Helper.DeltaInWorldToMapDelta(
                    location - pPos, -playerTerrainHeight + height);
        ...
        private void DrawMapIcons(Vector2 mapCenter, RadarFrame frame, float iconSizeMultiplier)
        {
            var fgDraw = ImGui.GetWindowDrawList();
            var clipMin = ImGui.GetWindowPos();
            var clipMax = clipMin + ImGui.GetWindowSize();
            ...
            foreach (var entityFrame in frame.Entities)
            {
                var entityValue = entityFrame.Entity;
                if (this.Settings.HideOutsideNetworkBubble && !entityFrame.IsValid)
                {
                    continue;
                }

                if (entityFrame.EntityState == EntityStates.Useless)
                {
                    continue;
                }

                var fpos = Helper.DeltaInWorldToMapDelta(
                    entityFrame.GridPosition - pPos,
                    entityFrame.TerrainHeight - frame.PlayerTerrainHeight);
                ...
                switch (entityFrame.EntityType)
                {
                    case EntityTypes.NPC:
                        DrawIcon(entityFrame.EntitySubtype == EntitySubtypes.SpecialNPC ? specialNpcIcon : npcIcon);
                        break;
                    case EntityTypes.Player:
                        if (entityFrame.EntitySubtype == EntitySubtypes.PlayerOther)
                        {
                            ...
                            {
                                DrawIcon(entityFrame.EntityState == EntityStates.PlayerLeader ? leaderIcon : playerIcon);
                            }
                        }
                        else
                        {
                    ...
                    case EntityTypes.Chest:
                        switch (entityFrame.EntitySubtype)
                        {
                            ...
                            case EntitySubtypes.ExpeditionChest:
                                if (entityFrame.Path.Contains("LeagueFaction") &&
                    ...
                    case EntityTypes.Monster:
                        switch (entityFrame.EntityState)
                        {
                            case EntityStates.None:
                                if (entityFrame.EntitySubtype == EntitySubtypes.POIMonster)
                                {
                                    if (!poiMonsterIcons.TryGetValue(entityFrame.EntityCustomGroup, out var poiIcon))
                                    {
                    ...
                    case EntityTypes.OtherImportantObjects:
                        if (entityFrame.EntityCustomGroup == RadarSettings.ExpeditionMarkerGroup)
                        {
                        ...
                        else if (entityFrame.EntityCustomGroup == RadarSettings.ExpeditionRemnantGroup)
                        {
                        ...
                        {
                            if (!otherImportantObjects.TryGetValue(entityFrame.EntityCustomGroup, out var mopoiIcon))
                            {
        ...
        private void WriteRadarMonitorFileIfNeeded(
            string mapType,
            Vector2 mapCenter,
            Vector2 shift,
            ...
            RadarFrame frame)
        {
            ...
            if ((now - this.lastRadarMonitorWriteTime).TotalSeconds < 1)
            {
                return;
            }

            this.lastRadarMonitorWriteTime = now;
            var previousCenter = mapType == "large" ? this.lastLargeMapCenter : this.lastMiniMapCenter;
            var centerDelta = previousCenter == Vector2.Zero ? 0f : Vector2.Distance(previousCenter, mapCenter);
            if (mapType == "large")
            {
                this.lastLargeMapCenter = mapCenter;
            }
            else
            {
                this.lastMiniMapCenter = mapCenter;
            }

            try
            {
                var entry = new RadarMonitorEntry(
                    now,
                    mapType,
                    mapCenter,
                    shift,
                    ...
                    zoom,
                    frame.PlayerGrid,
                    frame.PlayerTerrainHeight,
                    frame.Entities.Length,
                    centerDelta,
                    Core.Process.WindowArea.Location.X,
                    Core.Process.WindowArea.Location.Y,
                    Core.Process.WindowArea.Width,
                    Core.Process.WindowArea.Height);

                File.AppendAllText(
                    RadarMonitorPath,
                    System.Text.Json.JsonSerializer.Serialize(
                        entry,
                        new System.Text.Json.JsonSerializerOptions { IncludeFields = true }) + Environment.NewLine);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Radar.WriteRadarMonitorFileIfNeeded] {ex.Message}");
            }
        }
        ...
        private readonly record struct RadarFrame(
            Vector2 PlayerGrid,
            float PlayerTerrainHeight,
            RadarEntityFrame[] Entities);

        private readonly record struct RadarEntityFrame(
            Entity Entity,
            bool IsValid,
            EntityTypes EntityType,
            EntitySubtypes EntitySubtype,
            EntityStates EntityState,
            int EntityCustomGroup,
            string Path,
            Vector2 GridPosition,
            float TerrainHeight);

        private readonly record struct RadarMonitorEntry(
            DateTime Time,
            string MapType,
            Vector2 MapCenter,
            Vector2 Shift,
