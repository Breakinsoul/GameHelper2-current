...
namespace Launcher
{
    ...
    public static class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            {
                if (args.Length < 5 || !int.TryParse(args[3], out var launcherProcessId))
                {
                    Console.WriteLine("Invalid update arguments.");
                    Environment.ExitCode = 1;
                    return;
                }

                Environment.ExitCode = AutoUpdate.ApplyUpdateAndRestart(
                    args[1],
                    args[2],
                    launcherProcessId,
                    args[4]);
