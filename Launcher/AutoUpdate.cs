namespace Launcher
{
    ...
    public static class AutoUpdate
    {
        ...
        private static void CopyUpdatePayload(string extractedPayloadPath, string installDir)
        {
            ...
            {
                CopyUpdatePayloadEntry(entry, installDir, extractedPayloadPath);
            }
        }

        private static void CopyUpdatePayloadEntry(string sourcePath, string installDir, string payloadRoot)
        {
            var relativePath = Path.GetRelativePath(payloadRoot, sourcePath);
            if (ShouldPreserveUserPath(relativePath))
            {
                return;
            }
            ...
            if (Directory.Exists(sourcePath))
            {
                ...
                {
                    CopyUpdatePayloadEntry(entry, installDir, payloadRoot);
                }

                return;
            }

            Directory.CreateDirectory(Path.GetDirectoryName(destinationPath) ?? installDir);
            File.Copy(sourcePath, destinationPath, true);
        }

        private static bool ShouldPreserveUserPath(string relativePath)
        {
            ...
            if (parts[0].Equals("configs", StringComparison.OrdinalIgnoreCase) ||
            ...
            if (parts.Length == 1 &&
                (parts[0].Equals("tempFileLocations.dat", StringComparison.OrdinalIgnoreCase) ||
                 parts[0].Equals("imgui.ini", StringComparison.OrdinalIgnoreCase)))
            {
                return true;
            }

            if (parts[0].Equals("Plugins", StringComparison.OrdinalIgnoreCase))
            {
                if (parts.Any(part => part.Equals("config", StringComparison.OrdinalIgnoreCase)))
                {
                    return true;
                }
                ...
                if (parts.Length >= 3 &&
                    (fileName.Equals("important_tgt_files.txt", StringComparison.OrdinalIgnoreCase) ||
                     fileName.Equals("boss_arena_tgt_files.txt", StringComparison.OrdinalIgnoreCase) ||
                     fileName.Equals("stairs_tgt_files.txt", StringComparison.OrdinalIgnoreCase)))
                {
                    return true;
                }
            }
