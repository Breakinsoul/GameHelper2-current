namespace GameOffsets
{
    ...
    public struct Pattern
    {
        ...
        {
            var data = new StringBuilder($"Name: {this.Name} Pattern: ");
            for (var i = 0; i < this.Data.Length; i++)
            {
                if (this.Mask[i])
                {
                    data.Append(CultureInfo.InvariantCulture, $"0x{this.Data[i]:X} ");
                }
                else
                {
                    data.Append("?? ");
            ...
            data.Append(CultureInfo.InvariantCulture, $"BytesToSkip: {this.BytesToSkip}");
